    ...
    <form method="POST" action="{{ route('admin.testimonials.update', $testimonial) }}">
        ...
        <div class="mb-6">
            ...
            <select id="grade_level"
                ...
                <option value="Grade 7" {{ old('grade_level', $testimonial->grade_level) == 'Grade 7' ? 'selected' : '' }}>Grade 7</option>
                <option value="Grade 8" {{ old('grade_level', $testimonial->grade_level) == 'Grade 8' ? 'selected' : '' }}>Grade 8</option>
                <option value="Grade 9" {{ old('grade_level', $testimonial->grade_level) == 'Grade 9' ? 'selected' : '' }}>Grade 9</option>
                <option value="Grade 10" {{ old('grade_level', $testimonial->grade_level) == 'Grade 10' ? 'selected' : '' }}>Grade 10</option>
                <option value="Alumni" {{ old('grade_level', $testimonial->grade_level) == 'Alumni' ? 'selected' : '' }}>Alumni</option>
            </select>
            @error('grade_level')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        ...
        <div class="mb-6">
            ...
            <textarea id="message"
                      ...
                      placeholder="Enter the student's testimonial...">{{ old('message', $testimonial->message) }}</textarea>
            @error('message')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
